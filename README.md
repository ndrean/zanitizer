...
> Automatic bindings  [lxb->qjs] generated by running `zig run gen_bindings.zig` whenever possible (in `gen_bindings.zig`) with static functions (on 'document') and elements methods (on 'elemment' or 'node').

**WIP**:

> EventLoop + WorkerPool + Async Tasks. Promises created on main thread, resolved in worker pool (4?). Two queues, jobs and results. Use `libcurl` ?
