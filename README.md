...
1. Native Zig Event Loop: A custom, thread-safe loop handling Timers (Macrotasks) and Promises (Microtasks).
2. Worker Pool: CPU-intensive tasks (CSV parsing) running on background threads without blocking the main JS thread: pur `Zig` functions injection (eg statistics, CSV parsing etc) into quickJS usable code.
3. Memory Safety: A "Zero-Leak" architecture using Arena allocators for temporary work and precise hand-offs for results.
4. Binary Interop: Zero-copy passing of ArrayBuffers and efficient Tuples.
5. ES6 Module System: You are now loading external, third-party libraries (es-toolkit) from disk, resolving paths, handling extensions, and executing them natively.
