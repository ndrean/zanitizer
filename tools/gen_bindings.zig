...
const ArgType = union(enum) {
    ...
    node, // *DomNode from argv (passed as argument)
...
const api_functions = [_]BindingSpec{
    ...
    // JS: document.craeteTextNode(data: []const u8) !*DomNode
    .{
        .name = "createTextNode",
    ...
    .{
        ...
        .kind = .method,
        .args = &.{ .allocator, .this_node }, // Allocator is explicit!
...
fn countJSArgs(args: []const ArgType) u32 {
    ...
    for (args) |arg| {
        switch (arg) {
            .string, .int32, .uint32, .boolean, .node => count += 1,
