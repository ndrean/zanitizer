...
fn debugTabber(data: [*:0]const u8, _: usize, _: ?*anyopaque) callconv(.c) c_int {
    z.print("{s}|\t", .{data});
    return 0;
}
...
fn defaultStyler(data: [*:0]const u8, len: usize, context: ?*anyopaque) callconv(.c) c_int {
    ...
    if (ctx_ptr.expect_attr_value and ctx_ptr.found_equal) {
        ...
        if (is_dangerous) {
            ...
            applyStyle(ctx_ptr, z.SyntaxStyle.attr_value, text); // Normal styling
        }
...
fn applyStyle(ctx: *ProcessCtx, style: []const u8, text: []const u8) void {
    if (ctx.file) |f| {
        // FILE MODE: Strip colors, write clean text
        f.writer().print("{s}", .{text}) catch {};
    } else {
