...
pub fn main() !void {
    ...
    try streamingDemo(gpa, sandbox_root);
}

fn streamingDemo(gpa: std.mem.Allocator, sandbox_root: []const u8) !void {
    ...
    const js =
        ...
        // \\        const file = new File(["(file data on disk)"], FILE_NAME);
        // \\        file._path = FILE_NAME; // HACK: Your serialization might check this property?
        // \\                                // If not, we might need `fs.openAsFile(path)` helper.
        // \\        form.append("media", file, FILE_NAME);
        ...
        \\        } else {
        \\             console.log(`⚠️  Server didn't report the file (Size might be too big for HttpBin response)`);
        \\        }
